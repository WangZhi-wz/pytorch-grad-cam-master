...
from models.ACSNet_RFB_LLCA_CSAM_UACA.layers import conv
...
class UACA_end(nn.Module):
    ...
    def forward(self, x, map, lca_down):      # x(2, 256, 16, 16)  map(2, 1, 16, 16)
        ...
        # x = x * context + lca_down
        # return x

        # attx = residual * context
        # out = residual + attx + lca_down

        out = residual + context + lca_down

        return out


class UACA_modify(nn.Module):
    def __init__(self, in_channels, out_channels, in_channels_modify):
        ...
        self.convl = nn.Conv2d(in_channels_modify, in_channels_modify // 2, kernel_size=1, stride=1)
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear')
