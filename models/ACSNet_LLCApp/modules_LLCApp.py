...
class LCA4(nn.Module):
    ...
    def forward(self, x, pred, e_down):
        residual = x
        ...
        e_down = self.convl(e_down)
        e_down = self.upsample(e_down)


        out = att_x + residual + e_down
...
class LCA_32(nn.Module):
    def __init__(self, in_channels):
        super(LCA_32, self).__init__()
        self.convl = nn.Conv2d(in_channels, in_channels // 2, kernel_size=1, stride=1)
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear')

    #
    def forward(self, x, pred, lca_down, e_down):
        residual = x
        score = torch.sigmoid(pred)
        dist = torch.abs(score - 0.5)
        att = 1 - (dist / 0.5)
        ...
        out = att_x + residual + lca_down + e_down
