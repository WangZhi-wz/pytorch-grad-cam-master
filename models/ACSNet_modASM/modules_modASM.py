...
class ASMmod(nn.Module):
    def __init__(self, in_channels, rate = 4):
        super(ASMmod, self).__init__()
        self.non_local = NonLocalBlock(in_channels)
        ...
        self.spatial_attention = nn.Sequential(
            nn.Conv2d(in_channels*2, mid_channels, kernel_size=7, stride=1, padding=3),
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channels, in_channels*2, kernel_size=7, stride=1, padding=3),
    ...
    def forward(self, lc, fuse, gc):
        ...
        # channel attention
        b, c, h, w = fuse1.shape
        x_permute1 = fuse1.permute(0, 2, 3, 1).view(b, -1, c)
        x_att_permute1 = self.channel_attention(x_permute1).view(b, h, w, c)
        x_channel_att1 = x_att_permute1.permute(0, 3, 1, 2)
        fuse1 = fuse1 * x_channel_att1

        fuse2 = fuse + gc
        # channel attention
        b, c, h, w = fuse2.shape
        x_permute2 = fuse2.permute(0, 2, 3, 1).view(b, -1, c)
        x_att_permute2 = self.channel_attention(x_permute2).view(b, h, w, c)
        x_channel_att2 = x_att_permute2.permute(0, 3, 1, 2)
        fuse2 = fuse2 * x_channel_att2

        fuse = torch.cat([fuse1, fuse2], dim= 1)
        # spatial attention
        x_spatial_att = self.spatial_attention(fuse).sigmoid()
        out = fuse * x_spatial_att

        return out

class ASMmodend(nn.Module):
    def __init__(self, in_channels, rate = 4):
        super(ASMmodend, self).__init__()
        self.non_local = NonLocalBlock(in_channels)
        self.convl = nn.Conv2d(in_channels // 2, in_channels, kernel_size=1, stride=1)
        # self.selayer = SELayer(all_channels)
        mid_channels = in_channels // rate
