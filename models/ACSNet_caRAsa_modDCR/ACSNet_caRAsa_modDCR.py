...
from models.ACSNet_caRAsa_modDCR.modules_caRAsa_modDCR import GCM, ASM, GCM_RFB, ASM_nogcm, CSAMBlock, \
    LCA11, LCA32, LCA44, RA1, RA23, RA4, NonLocalBlock, CSAMBlock1, CSAMBlock432, GCM_RFB_s, GCM_CBAM23, \
    GCM_CBAM1, GCM_CBAM4, GCM_CBAM, ChannelAttention, SpatialAttention, SELayer, ASMmodend, ASMmod, \
    ChannelAttention_mod, ChannelAttention_modeca, ChannelAttention_gam, ChannelAttention_gam1, ChannelAttention_gam12, \
    ChannelAttention_gam123
from models.ACSNet_caRAsa_modDCR.modules import conv2d, conv1d, PAM_Module, CAM_Module
...
class ACSNet_caRAsa_modDCR(nn.Module):
    def __init__(self,
                 bank_size=20,
                 num_classes=1,
                 num_channels=3,
        ...
        super(ACSNet_caRAsa_modDCR, self).__init__()
        ...
        self.pool5 = nn.Sequential(  # Global average pool -> 3x3AdaptiveAvgPool -> 5x5AdaptiveAvgPool
            ...
            nn.Conv2d(512, 128, 1, 1),
            nn.ReLU(inplace=True))
        self.pool7 = nn.Sequential(  # Global average pool -> 3x3AdaptiveAvgPool -> 5x5AdaptiveAvgPool
        ...
        # RA
        self.lca1 = RA1(64)
        self.lca2 = RA23(64, 128)
        self.lca3 = RA23(128, 256)
        self.lca4 = RA4(256)
        # ca_LLCA_sa
        # local context attention module
        # self.lca4 = LCA44(256)
    ...
    def region_representation(self, input):  # e5(4,512,7,7)
        ...
        X1 = self.pool1(input)
        X1 = F.interpolate(X1, 8, mode='bilinear', align_corners=True)
        X2 = self.pool3(input)
        X2 = F.interpolate(X2, 8, mode='bilinear', align_corners=True)
        X3 = self.pool5(input)
        X3 = F.interpolate(X3, 8, mode='bilinear', align_corners=True)
        X4 = self.pool7(input)
        ...
        f_k = (M @ X_flat_old.transpose(1, 2)).transpose(1, 2)  # 上下文区域嵌入公式 (4,512,1)
    ...
    def attentive_interaction(self, bank, X_flat, X):
        ...
        # bank(4, 512, 1)    X_flat = feats_flat = (4, 512, 49)     X = feats = (4, 512, 7, 7)
        batch, n_class, height, width = X.shape  # batch=4, n_class=512 , height=7, width=7
        # query = S * C 缓冲区域嵌入
        query = bank.squeeze(dim=2)  # q：压缩 (4, 512, 1)->(4,512)
        # key: = B * C * HW 扁平化特征
        key = X_flat  # (4,512,49) -> (4,256,49)

        # logit = HW * S * B (cross image relation) 在它们之间执行矩阵乘法
        logit = torch.matmul(query, key).transpose(0, 2)  # (49,4,4)
        # attn = HW * S * B 应用 softmax 层来计算上下文注意力图 X ∈ RHW×B×S
        attn = torch.softmax(logit, 2)  # softmax维度要正确 (49,4,4)

        # delta = S * C
        delta = self.delta(bank).squeeze(dim=2)  # (4,256)
        # attn_sum = B * C * HW
        attn_sum = torch.matmul(attn.transpose(1, 2), delta).transpose(1, 2)  # (49,256,4)
        # x_obj = B * C * H * W
        X_obj = self.rho(attn_sum).view(batch, -1, height, width)  # (4, 512, 7, 7)
